...
import {AddCircle, CheckCircle, Highlight} from "@material-ui/icons";
...
function Links(props) {
  ...
  function handleClick(event, tlink) {
    ...
    if(dictionary != undefined && dictionary[tlink] !== undefined) {
...
//         <TableRow key={hd_1}>
//           <TableCell>
//             <b>{currgroup.headline}</b>
//           </TableCell>
//         </TableRow>
//         <TableRow key={hd_2}>
//           <TableCell>
//             <ItemsRender dic={props.dic} items={currgroup.items} />
//           </TableCell>
//         </TableRow>
//       </TableBody>
//     </Table>
...
function XTable(props) {
  ...
  // let currgroup = content.currentGroup;
  ...
  return (
    <div style={{backgroundColor: '#fff'}}>
      ...
      {/*<br/>*/}
      {/*<div><b>In addition</b></div>*/}
      {/*<CurrGroupRender dic={props.dic} currgroup={currgroup} />*/}
    </div>
  ) 
}
      ...
      if(eng.stage !== "") {
        if(lastNode === null || lastNode.stage != eng.stage) {
          ...
          if(lastNode.category != eng.category) {
            ...
            getLastStage(englishTree).categories.push(category);
          } else {
            if(lastNode.groupTitle != eng.groupTitle) {
              ...
              getLastCategory(getLastStage(englishTree).categories).groups.push(group);
            } else {
              // console.log("Group Title ==: (" + lastNode.groupTitle + ", " + eng.groupTitle + ")");

              getLastGroup(getLastCategory(getLastStage(englishTree).categories).groups).objects.push({
                outcomes: eng.outcomes,
                content: eng.content
              })
            }
...
export default function EnglishK10SyllabusTreeView(props) {
  const classes = useStyles();
  ...
  return (
    ...
    >
      <TreeItem nodeId="o-0" label="English K-10 Syllabus">
        {
          data.map(
            stage => 
              <StageRender key={count++} dic={props.dic} terms={props.terms} stage={stage} oindex="o-0" index={oc++} />
          )
        }
      </TreeItem>
    </TreeView>
