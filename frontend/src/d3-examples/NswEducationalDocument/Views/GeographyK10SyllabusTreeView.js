    ...
    eng => {
      if(eng.stage !== "") {
        if(lastNode === null || lastNode.stage != eng.stage) {
          ...
          let category = {
            title: (eng.category !== undefined && eng.category !== null && eng.category !== "") ? eng.category : "Category",
            groups: []
          }
          ...
          if(lastNode.category != eng.category) {
            ...
            getLastStage(geographyTree).categories.push(category);
          } else {
            if(lastNode.groupTitle != eng.groupTitle) {
              ...
              getLastCategory(getLastStage(geographyTree).categories).groups.push(group);
            } else {
              console.log("Group Title ==: (" + lastNode.groupTitle + ", " + eng.groupTitle + ")");

              getLastGroup(getLastCategory(getLastStage(geographyTree).categories).groups).objects.push({
                outcomes: eng.outcomes,
                content: eng.content
              })
            }
...
export default function GeographyK10SyllabusTreeView(props) {
  const classes = useStyles();
  ...
  return (
    ...
    >
      <TreeItem nodeId="o-0" label="Geography K-10 Syllabus">
        {
          data.map(
            stage => 
