...
function XTable(props) {
  ...
  let code = "";
  if(outcomes.outcomes[0] !== undefined && outcomes.outcomes[0] !== null && outcomes.outcomes[0].code !== undefined) {
    code = outcomes.outcomes[0].code;
  }

  return (
    <div>
      <ViewToolbar label={code} />
...
function convertData() {
    ...
    eng => {
      if(eng.stage !== "") {
        if(lastNode === null || lastNode.stage != eng.stage) {
          ...
          let stageStr = eng.state;
          console.log("Stage title: " + stageStr);
          if(stageStr === null || stageStr === undefined) {
            stageStr = "Stage";
          }
          ...
          if(lastNode.category != eng.category) {
            ...
            getLastStage(chemistryTree).categories.push(category);
          } else {
            if(lastNode.groupTitle != eng.groupTitle) {
              ...
              getLastCategory(getLastStage(chemistryTree).categories).groups.push(group);
            } else {
              console.log("Group Title ==: (" + lastNode.groupTitle + ", " + eng.groupTitle + ")");

              getLastGroup(getLastCategory(getLastStage(chemistryTree).categories).groups).objects.push({
                outcomes: eng.outcomes,
                content: eng.content
              })
            }
  ...
  return chemistryTree;
}

export default function ChemistryK10SyllabusTreeView(props) {
  const classes = useStyles();
  ...
  return (
    ...
    >
      <TreeItem nodeId="o-0" label="Chemistry K-10 Syllabus">
        {
          data.map(
            stage => 
