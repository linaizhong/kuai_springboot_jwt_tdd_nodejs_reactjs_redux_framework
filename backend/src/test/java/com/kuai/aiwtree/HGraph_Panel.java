...
public class HGraph_Panel extends Panel
{
    ...
    private Hyperbolic_Tree parsingInfo(String s)
    {
        Hyperbolic_Tree h_t = null;
        ...
        try
        {
            ...
            if(k > 0)
            {
                ...
                if(!s1.equals(""))
                {
                    ...
                    h_t = new Hyperbolic_Tree(node);
                    h_t.addNode(s3, s5, s7);
                }
                i = k + 1;
                k = s.indexOf(byte0, i);
                if(k >= 0)
